...
type KittyRow = 
    { Date:DateTime; Item:string; SpentPaids:SpentPaid list }
...
let kittyRows (accounts : Map<int,Account>) journals = 
    let CreateSpentPaid ts (header,accs) = 
        ...
        { Header = header; Spent = sumTrans In ts; Paid = sumTrans Out ts }
...
let kittySummaries accounts journals = 
    ...
    kittyRows accounts journals |> Seq.map (fun (ac,krs) -> ac,kittySummary "Total" krs)
...
let kittySummaryStrings accounts journals = 
    ...
    kittySummaries accounts journals |> Seq.map (fun (ac,ks) -> ac,kittySummaryString ac ks)
